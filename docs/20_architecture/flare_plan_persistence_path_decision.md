...
| --- | --- | --- | --- | --- | --- |
| Flare Events | `frontend/src/services/flareEventRepository.ts` | Frontend Supabase client | Supabase JS over HTTPS/PostgREST | End-user session with RLS | Single-table reads and writes; no backend-managed row locking |
| Behavior Patterns | `frontend/src/services/behaviorPatternRepository.ts` | Frontend Supabase client | Supabase JS over HTTPS/PostgREST | End-user session with RLS | Simple upsert-style reads and writes |
| Anchor Notes | `frontend/src/services/anchorNoteRepository.ts` | Frontend Supabase client | Supabase JS over HTTPS/PostgREST | End-user session with RLS | Simple upsert-style reads and writes |
